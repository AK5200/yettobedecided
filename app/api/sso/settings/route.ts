...
export async function GET() {
  ...
  const { data: org } = await supabase
    .from('organizations')
    .select('sso_mode, sso_secret_key, guest_posting_enabled, social_login_enabled, sso_redirect_enabled, sso_redirect_url')
    .eq('id', member.org_id)
    .single();
  ...
  return NextResponse.json({
    ...
    social_login_enabled: org.social_login_enabled ?? true,
...
export async function PUT(request: Request) {
  ...
  if (typeof social_login_enabled === 'boolean') updateData.social_login_enabled = social_login_enabled;
